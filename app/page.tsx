...
const quickActions = [
  ...
  { label: "Find Info", search: "search" },
  { label: "Make a Movie", search: "movie" },
  { label: "Write Code", search: "code" },
  { label: "Create Song", search: "song" }
];
    ...
    <main className="min-h-screen bg-[#020617] text-slate-100 font-sans">
      <div className="max-w-6xl mx-auto px-6 pt-24 pb-16 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-16">
          {quickActions.map(action => (
            <button key={action.label} onClick={() => {setSearchQuery(action.search); setSelectedCategory('All Tools');}} className="px-6 py-3 rounded-2xl bg-slate-900/50 border border-slate-800 text-xs font-black uppercase tracking-widest hover:border-indigo-500 transition-all">{action.label}</button>
          ))}
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-24">
        ...
        <section id="ranking-table" className="mt-40 border-t border-slate-900 pt-32 pb-48">
          <div className="text-center mb-20"><BarChart3 className="w-16 h-16 text-indigo-500 mx-auto mb-6" /><h2 className="text-6xl font-black italic tracking-tighter mb-4 uppercase">Power Rankings</h2><p className="text-slate-500 font-black uppercase tracking-[0.5em] text-xs italic">Verified Scores</p></div>
